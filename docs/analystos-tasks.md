...
| T5.2 | Implement `/db-document` command | Interactive table or field documentation flow. Runs introspection, presents draft to analyst, accepts corrections/additions, writes final entry to `db-knowledge/{connection-name}/{schema}/{table}.md` | Should detect and offer to update an existing entry rather than overwrite silently |
| T5.3 | Implement `/db-capture` command | Saves the current query + analyst-provided context (purpose, result interpretation, caveats) as a named `.sql` file in `db-knowledge/{connection-name}/{schema}/_queries/`. Prompts for name and description | Lightweight — no introspection required |
